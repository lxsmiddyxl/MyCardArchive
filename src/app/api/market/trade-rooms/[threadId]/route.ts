...
const CTX = { componentName: "api/market/trade-rooms", surfaceName: "marketplace" } as const;
...
async function GET_handler(_request: Request, context: { params: Record<string, string> }) {
  ...
  const { data: room } = await supabase.from("market_trade_rooms").select("*").eq("thread_id", threadId).maybeSingle();

  const { data: revisions, error: rErr } = await supabase
    .from("market_offer_revisions")
    .select("id, thread_id, seq, offer_id, snapshot, actor_id, created_at")
    .eq("thread_id", threadId)
  ...
  mcaLog.event("market.trade_room.open", { threadId, viewerId: user.id, revisionCount: revisions?.length ?? 0 }, CTX);

  return NextResponse.json({
    room: room ?? { thread_id: threadId, created_at: list[0]?.created_at, updated_at: list[0]?.updated_at },
    offers: list,
    revisions: revisions ?? [],
    events: events ?? [],
  });
...
export const GET = defineRoute("GET /api/market/trade-rooms/[threadId]", GET_handler);
