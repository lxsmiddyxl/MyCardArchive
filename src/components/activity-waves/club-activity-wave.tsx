...
import type { ClubActivityWavePayloadDTO, SetClubWaveHourDTO } from "@/lib/dto/activity-waves";
...
function barTone(band: string): string {
  switch (band) {
    case "very_active":
      return "bg-mca-accent-strong/50";
    case "active":
      return "bg-mca-accent/45";
    case "steady":
...
export function ClubActivityWave({ clubId, className }: { clubId: string; className?: string }) {
  const [bootstrapped, setBootstrapped] = useState(false);
  const { run, data, loading } = useAsyncState<ClubWaveView>();

  const load = useCallback(() => {
    return run(async () => {
      const r = await fetchJson<ClubActivityWavePayloadDTO>(
        `/api/activity-waves/club?clubId=${encodeURIComponent(clubId)}`,
        { cache: "no-store" }
      );
    ...
    <Panel className={cn("border border-mca-border/80 bg-mca-surface-elevated/35 p-mca-md", className)}>
      <section aria-live="polite" aria-busy={loading || !bootstrapped}>
        ...
        <p className="mt-mca-xs text-sm font-medium text-mca-ink-body">
          {!bootstrapped ? "Arc loading…" : headline || "Arc snapshot unavailable."}
        </p>
        <p className="mt-mca-micro text-mca-caption text-mca-ink-muted">
          Ambient overlap from club rooms — no chat or leaderboards.
        </p>
